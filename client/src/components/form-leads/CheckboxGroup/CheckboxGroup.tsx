...
type CheckboxGroupProps<T extends FieldValues>  = {
  ...
  const CheckboxGroup = <T extends FieldValues> ({ title, name, options, register, errors }: CheckboxGroupProps<T>) =>  {
  ...
  {error?.message && (
    <p className="text-[10px] font-poppins text-red-500 mt-1 px-2 py-1 rounded">
      {error.message}
    </p>
  )}
</fieldset>
