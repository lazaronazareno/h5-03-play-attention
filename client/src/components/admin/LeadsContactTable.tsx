...
const LeadsContactTable = ({ type, lead }: LeadsContactTableProps) => {
      ...
      <table className="table-auto w-full border-collapse">
        ...
        <tbody>
          <tr>
            ...
            <td className=" px-4 py-2">{LeadTypeNames[lead.leadType]}</td>
            <td className=" px-4 py-2">{LeadStatusNames[lead.status]}</td>
      ...
      {openSendEmailForm && (
        <div className='fixed top-0 right-0 w-full h-full bg-black/50 flex justify-end z-50' onClick={() => setOpenSendEmailForm(false)}>
          <div className='relative flex flex-col gap-4 bg-neutral-white2 border border-violet-main rounded-md shadow-main' onClick={(e) => e.stopPropagation()}>
            <EmailForm type={type} users={[lead.email]} onClick={setOpenSendEmailForm} />
          </div>
        </div>
      )}
    </div>
...
export default LeadsContactTable;